...
var Job = function(jobid, config) {
  ...
  _.each(self.dag.tasks, function(tid) { 
    ...
    if(!config.jobs[tid].executable && !config.jobs[tid].javascript) {
      throw Error('executable/javascript should be specified:'+tid);
    }
    if(config.jobs[tid].executable && config.jobs[tid].javascript) {
      throw Error('only one of executable/javascript should be specified:'+tid);
    }
    var jname = config.jobs[tid].name || '';
    if(config.jobs[tid].executable) {
      self.q.add(new task.Task(jobid, tid, jname, config.jobs[tid].executable));
    } else {
      var exec;
      self.q.add(new task.Task(jobid, tid, jname, exec,
                               config.jobs[tid].javascript.lib,
                               config.jobs[tid].javascript.method));
    }
  });
...
Job.prototype.isComplete = function() {
  ...
  if(id) return false;
  // check if results have same # of tasks
  if(_.size(this.dag.tasks) == _.size(this.results)) return true;
  return false;
};
